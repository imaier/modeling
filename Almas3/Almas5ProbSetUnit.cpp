...
//---------------------------------------------------------------------------
TAlmas5ProbSetGlobal g_Almas5ProbSetGlobal;
//---------------------------------------------------------------------------
TAlmas5ProbSetGlobal::TAlmas5ProbSetGlobal()
{
...
//---------------------------------------------------------------------------
void __fastcall TAlmas5ProbSetKey::fill(TProbKey nKey)
{
//#define Adg1Base 1
//количество первых соседей
//#define Adg1State 3
...
//#define divSetAdg2(Key, Data, i, j) (Key + ((Data%Adg2State) - divGetAdg2(Key,i,j))*Adg2Base(i,j))
	...
	int nType;
	int nNumber=-2;
	TAlmas5ProbSetGlobal::GetProbNameFromKey2(nKey, nType, nNumber, strName);
}
//---------------------------------------------------------------------------
TProbKey __fastcall TAlmas5ProbSetKey::unfill()
{
	TProbKey nKey = 0;
...
//---------------------------------------------------------------------------
TProbKey __fastcall TAlmas5ProbSetGlobal::RemoveLeftFromKey2(TProbKey BaseKey)
...
//---------------------------------------------------------------------------
TProbKey __fastcall TAlmas5ProbSetGlobal::CanonateKey2(TProbKey BaseKey)
{
	TProbKey nKey = BaseKey;
...
//---------------------------------------------------------------------------
bool __fastcall TAlmas5ProbSetGlobal::ChekKey2(TProbKey nKey)
...
//---------------------------------------------------------------------------
TProbKey __fastcall TAlmas5ProbSetGlobal::KeyToKey2(TProbKey BaseKey)
{
	TAlmas5ProbSetKey psk;
	psk.fill2(BaseKey);
	return psk.Key2;
}
//---------------------------------------------------------------------------
void TAlmas5ProbSetGlobal::Init(void)
...
//---------------------------------------------------------------------------
AnsiString TAlmas5ProbSetGlobal::GetProbNameFromKey(TProbKey nKey)
{
	AnsiString strName = "Не реализовано";
	return strName;
}
//---------------------------------------------------------------------------
void __fastcall TAlmas5ProbSetGlobal::GetProbNameFromKey2(TProbKey nKey, int &nType, int &nNumber, AnsiString& strName)
{
/*
 ...
 */
	...
	strName.sprintf("%02d. (s%d, v%d; s%d, ns%d)x%d - %d", nNumber+1, psk.n1s, psk.n1v, psk.n2s, psk.nns, nType, nKey);
}
//---------------------------------------------------------------------------
int __fastcall TAlmas5ProbSetGlobal::GetTypeFromKey2(TProbKey BaseKey)
{
	int nType = 0;
...
//---------------------------------------------------------------------------
TAlmas5ProbSetGlobal& TAlmas5ProbSetGlobal::Get(void)
{//получение ссылки на глобальный класс
	return g_Almas5ProbSetGlobal;
}
//---------------------------------------------------------------------------
TProbIndex TAlmas5ProbSetGlobal::GetProbIndexFromAtomShema(TAtomShema &as)
{//получение индекса из набора для данного расположения атомов
	TProbIndex nPI = PI_ERROR;
	if(as.m_nMaxLayer < m_nNumLayerAtomShema)
	{
		throw Exception("TAlmas5ProbSetGlobal::GetProbIndexFromAtomShema - атомсхема описана недостаточно полно.");
	}
	if(as.GetBit(GetIndex0LevAdj()) == 1)
	{
		...
		{
		 TProbKey2 nKey2;
		 SetKey2(nKey2, n1s, n1v, n2s, nns);// формируем ключ2 (n1s+n1v+n2s+n2v)
		 TKey2ToKeyMap::iterator itr;
...
//---------------------------------------------------------------------------
int TAlmas5ProbSetGlobal::Get_nS_CountForProb(int Index)
...
//---------------------------------------------------------------------------
IBaseProbSet* TAlmas5ProbSetCreator::Create()
{//создание набора вероятностей
	TAlmas5ProbSet *pNewProbSet = new TAlmas5ProbSet(*this);
	return pNewProbSet;
}
//---------------------------------------------------------------------------
TAlmas5ProbSet::TAlmas5ProbSet(IBaseProbSetCreator& Creator)
