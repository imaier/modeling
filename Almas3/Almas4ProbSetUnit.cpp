...
//---------------------------------------------------------------------------
TAlmas4ProbSetGlobal g_Almas4ProbSetGlobal;
//---------------------------------------------------------------------------
TAlmas4ProbSetGlobal::TAlmas4ProbSetGlobal()
{
...
//#define divSetAdg2(Key, Data, i, j) (Key + ((Data%Adg2State) - divGetAdg2(Key,i,j))*Adg2Base(i,j))
	...
	int nType;
	int nNumber=-2;
	TAlmas4ProbSetGlobal::GetProbNameFromKey2(nKey, nType, nNumber, strName);
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
TProbKey __fastcall TAlmas4ProbSetGlobal::RemoveLeftFromKey2(TProbKey BaseKey)
...
//---------------------------------------------------------------------------
TProbKey __fastcall TAlmas4ProbSetGlobal::CanonateKey2(TProbKey BaseKey)
{
	TProbKey nKey = BaseKey;
...
//---------------------------------------------------------------------------
bool __fastcall TAlmas4ProbSetGlobal::ChekKey2(TProbKey nKey)
...
//---------------------------------------------------------------------------
TProbKey __fastcall TAlmas4ProbSetGlobal::KeyToKey2(TProbKey BaseKey)
{
	TAlmas4ProbSetKey psk;
	psk.fill2(BaseKey);
	return psk.Key2;
}
//---------------------------------------------------------------------------
void TAlmas4ProbSetGlobal::Init(void)
...
//---------------------------------------------------------------------------
AnsiString TAlmas4ProbSetGlobal::GetProbNameFromKey(TProbKey nKey)
{
	AnsiString strName = "Не реализовано";
	return strName;
}
//---------------------------------------------------------------------------
void __fastcall TAlmas4ProbSetGlobal::GetProbNameFromKey2(TProbKey nKey, int &nType, int &nNumber, AnsiString& strName)
{
/*
 ...
 */
	...
	strName.sprintf("%02d. (s%d, v%d; s%d, v%d, ns%d)x%d - %d", nNumber+1, psk.n1s, psk.n1v, psk.n2s, psk.n2v, psk.nns, nType, nKey);
}
//---------------------------------------------------------------------------
int __fastcall TAlmas4ProbSetGlobal::GetTypeFromKey2(TProbKey BaseKey)
{
	int nType = 0;
...
//---------------------------------------------------------------------------
TAlmas4ProbSetGlobal& TAlmas4ProbSetGlobal::Get(void)
{//получение ссылки на глобальный класс
	return g_Almas4ProbSetGlobal;
}
//---------------------------------------------------------------------------
TProbIndex TAlmas4ProbSetGlobal::GetProbIndexFromAtomShema(TAtomShema &as)
{//получение индекса из набора для данного расположения атомов
	TProbIndex nPI = PI_ERROR;
	if(as.m_nMaxLayer < m_nNumLayerAtomShema)
	{
		throw Exception("TAlmas4ProbSetGlobal::GetProbIndexFromAtomShema - атомсхема описана недостаточно полно.");
	}
	if(as.GetBit(GetIndex0LevAdj()) == 1)
	{
		...
		{
		 TProbKey2 nKey2;
		 SetKey2(nKey2, n1s, n1v, n2s, n2v, nns);// формируем ключ2 (n1s+n1v+n2s+n2v)
		 TKey2ToKeyMap::iterator itr;
...
//---------------------------------------------------------------------------
int TAlmas4ProbSetGlobal::GetMainAtomTypeForProb(int Index)
...
//---------------------------------------------------------------------------
int TAlmas4ProbSetGlobal::Get_nS_CountForProb(int Index)
...
//---------------------------------------------------------------------------
TAlmas4ProbSetCreator::TAlmas4ProbSetCreator()
:IBaseProbSetCreator(TAlmas4ProbSetGlobal::Get())
{
}
//---------------------------------------------------------------------------
IBaseProbSet* TAlmas4ProbSetCreator::Create()
{//создание набора вероятностей
	TAlmas4ProbSet *pNewProbSet = new TAlmas4ProbSet(*this);
	return pNewProbSet;
}
//---------------------------------------------------------------------------
TAlmas4ProbSet::TAlmas4ProbSet(IBaseProbSetCreator& Creator)
