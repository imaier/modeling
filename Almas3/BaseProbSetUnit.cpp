...
{//1-атом есть, 0 - атом удалЄн
	...
	{
		throw Exception("TAtomShema::GetBit = ¬ыход за пределы атомсхемы.");
	}
	return nState;
}
//---------------------------------------------------------------------------
...
{//1-атом есть, 0 - атом удалЄн
	...
	{
		throw Exception("TAtomShema::SetBit = ¬ыход за пределы атомсхемы.");
	}
}
//---------------------------------------------------------------------------
...
{//количество уникальных веро€тностей веро€тностей в наборе
	return m_nAllNumProbality;
}
//---------------------------------------------------------------------------
AnsiString IBaseProbSetGlobalData::GetProbName(int Index)
{//получение названи€ (имени) внро€тности
...
{//получение количества слоЄв до которого должна быть описана атомсхема
...
:m_Creator(Creator), m_GD(Creator.GetInfo()), m_pdProbArr(NULL)
{
	...
	//m_GD.FillDefaultProb(m_pdProbArr);//заполнить дефолтными веро€тност€ми
}
//---------------------------------------------------------------------------
...
{//получение указател€ на класс создател€
	return m_Creator;
}
//---------------------------------------------------------------------------
bool IBaseProbSet::FillDefault(void)
{//заполнить значени€ веро€тности по умолчанию
	return m_GD.FillDefaultProb(m_pdProbArr);//заполнить дефолтными веро€тност€ми
}
//---------------------------------------------------------------------------
bool IBaseProbSet::Copy(IBaseProbSet& ProbSet)
{//заполнить значени€ веро€тности из другого точно такогоже класса
    bool bRet = false;
...
{//установка значени€ веро€тности
	...
	if(dProbVal < 0)
	{
		throw Exception("ќтрицательное значение веро€тности недопустимо.");
    }
...
{//получение ссылки на клас глобальных данных дл€ этого набора
	return m_GD;
}
//---------------------------------------------------------------------------
